...
import (
	...
	"github.com/mhpenta/jobqueue/backend/postgresqueue/db"
)

// ListPendingJobs returns uncompleted jobs ordered by priority DESC, created_at ASC.
func (q *PostgresQueue) ListPendingJobs(ctx context.Context, queueName string, limit, offset int) ([]jobqueue.Message, error) {
...
func (q *PostgresQueue) ListCompletedJobs(ctx context.Context, queueName string, limit, offset int) ([]jobqueue.Message, error) {
...
func (q *PostgresQueue) ListDLQJobs(ctx context.Context, queueName string, limit, offset int) ([]jobqueue.DLQMessage, error) {
	...
	for i, row := range rows {
		msgs[i] = *dlqMessageFromRow(row)
	}
	return msgs, nil
}

// CountPendingJobs returns the number of uncompleted jobs in the queue.
func (q *PostgresQueue) CountPendingJobs(ctx context.Context, queueName string) (int64, error) {
...
func (q *PostgresQueue) CountCompletedJobs(ctx context.Context, queueName string) (int64, error) {
	return q.queries.CountCompletedJobs(ctx, queueName)
}

// CountDLQJobs returns the number of jobs in the dead-letter queue.
func (q *PostgresQueue) CountDLQJobs(ctx context.Context, queueName string) (int64, error) {
	return q.queries.CountDLQJobs(ctx, queueName)
}

// CleanupCompletedJobs deletes completed jobs older than the given time.
func (q *PostgresQueue) CleanupCompletedJobs(ctx context.Context, olderThan time.Time) error {
...
func (q *PostgresQueue) SweepStuckJobs(ctx context.Context, queueName string) (int, error) {
...
func (q *PostgresQueue) DeleteDLQJob(ctx context.Context, jobID string) error {
